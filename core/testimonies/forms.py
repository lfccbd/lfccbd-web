...
options = (
    (
        'Mr / Mrs / Sis / Bro / Deacon / Deaconess',
        'Mr / Mrs / Sis / Bro / Deacon / Deaconess',
    ),
    ('Mr', 'Mr'),
    ('Mrs', 'Mrs'),
...
class TestimonyForm(forms.ModelForm):
    first_name = Nh3CleanCharField(
        ...
        widget=forms.TextInput(
            attrs={'placeholder': 'Your First name', 'class': 'form-control'}
        ),
    )
    last_name = Nh3CleanCharField(
        required=True,
        widget=forms.TextInput(
            attrs={'placeholder': 'Your last name', 'class': 'form-control'}
        ),
    )
    designation = forms.CharField(
        required=True,
        widget=forms.Select(choices=options, attrs={'class': 'form-select'}),
    )
    title = Nh3CleanCharField(
        required=True,
        widget=forms.TextInput(
            attrs={'placeholder': 'Testimony title', 'class': 'form-control'}
        ),
    )
    testimony = Nh3CleanCharField(
        required=True, widget=forms.Textarea(attrs={'class': 'form-control', 'row': 6})
    )
    location = Nh3CleanCharField(
        required=True,
        widget=forms.TextInput(
            attrs={'placeholder': 'Winner Branch or City ', 'class': 'form-control'}
        ),
    )
    ...
    def clean_message(self):
        ...
        if user_tesitmonies.strip() == '':
            ...
            raise forms.ValidationError(
                'Length must be less than 800 character', code='testimony'
            )
    ...
    def clean_designation(self):
        ...
        if selected_designation == 'Mr / Mrs / Sis / Bro / Deacon / Deaconess':
